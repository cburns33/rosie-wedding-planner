...
3. **Existing projects:** re-run only the migration blocks you are missing from `schema.sql`. **Run each block in full, including its `GRANT` / `REVOKE` lines.** Every table here is RLS-protected and reachable only via the `service_role` key; creating a table without its grants leaves server-side reads/writes failing with `permission denied`, and the failure can be silent.
   ...
   - **`messages` sequence grant** (bottom of file): `messages.id` is a `bigserial`, so `service_role` also needs `GRANT USAGE, SELECT ON SEQUENCE public.messages_id_seq`. The table grant alone is not enough; without the sequence grant every INSERT fails with `permission denied for sequence messages_id_seq` and chat history silently never saves. (`wedding_state`, `inspiration_memory`, and `vendor_memory` use fixed/text keys, so only `messages` needs this.) This was the real cause of the empty `messages` table; fixed via a migration on 2026-06-26.
